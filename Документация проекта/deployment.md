...
В папке проекта создайте файл `.env` (без имени, только расширение).

**Способ 1: Через Блокнот**
1. Нажмите правой кнопкой в папке проекта → "Создать" → "Текстовый документ"
2. Назовите его `.env` (удалите `.txt` если есть)
3. Если Windows не даёт создать файл с точкой в начале:
   - Создайте файл `env.txt`
   - Откройте его, вставьте содержимое ниже
   - Сохраните и переименуйте в `.env`

**Способ 2: Через терминал**
1. Откройте терминал в папке проекта
2. Введите: `notepad .env`
3. Вставьте содержимое и сохраните

### 2. Настройте переменные окружения

Откройте файл `.env` в Блокноте и вставьте:
...
DB_PASSWORD=asteroid_password  # Ваш пароль из Шага 6!
DB_NAME=asteroid_db
...
- `DB_PASSWORD` — пароль который вы задали пользователю `asteroid_user`
- `DB_USER` — имя пользователя (если другое)
- `DB_NAME` — имя базы данных (если другое)

### 3. Сохраните файл .env

Нажмите `Ctrl + S` и закройте файл

**⚠️ Важно:** Файл `.env` содержит чувствительные данные (пароли). Никогда не коммитьте его в Git!

### 4. Проверьте config.yaml

Файл `config.yaml` уже содержит настройки NASA API и приложения. **Менять его не нужно**, если вы не знаете что делаете.

В нём хранятся:
- Настройки подключения к NASA API
- Настройки приложения (логирование, интервалы обновлений)

**Параметры базы данных в config.yaml НЕ указываются!** Они берутся только из `.env`.
...
2. Проверьте пароль в файле `.env` (переменная `DB_PASSWORD`)
3. Проверьте что база данных `asteroid_db` создана
4. Убедитесь что все переменные БД указаны в `.env` (DB_HOST, DB_PORT, DB_USER, DB_PASSWORD, DB_NAME)
...
### Файлы конфигурации:
- **`.env`** — параметры подключения к базе данных (DB_HOST, DB_PORT, DB_USER, DB_PASSWORD, DB_NAME)
- **`config.yaml`** — настройки NASA API и приложения
