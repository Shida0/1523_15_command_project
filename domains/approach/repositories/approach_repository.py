...
class ApproachRepository(BaseRepository[CloseApproachModel]):
    ...
    async def get_by_asteroid(self, asteroid_id: int, skip: int = 0, limit: Optional[int] = 1000) -> List[CloseApproachModel]:
        """Получает все сближения для конкретного астероида"""
        return await self.filter({"asteroid_id": asteroid_id}, skip, limit, order_by="approach_time")

    async def get_by_asteroid_designation(self, designation: str, skip: int = 0, limit: Optional[int] = 1000) -> List[CloseApproachModel]:
        """Получает все сближения для астероида с указанным обозначением"""
        return await self.filter({"asteroid_designation": designation}, skip, limit, order_by="approach_time")

    async def get_approaches_in_period(self, start_date: datetime, end_date: datetime, max_distance: Optional[float] = None, skip: int = 0, limit: Optional[int] = 1000) -> List[CloseApproachModel]:
        ...
        return await self.filter(filters, skip, limit, order_by="approach_time")

    async def get_upcoming_approaches(self, limit: Optional[int] = None, skip: int = 0) -> List[CloseApproachModel]:
        """Получает все сближения из базы данных. Возвращает ВСЕ сближения без фильтрации по времени"""
        return await self.filter({}, skip, limit, order_by="approach_time")

    async def get_closest_approaches_by_distance(self, limit: Optional[int] = None, skip: int = 0) -> List[CloseApproachModel]:
        """Получает самые близкие по расстоянию сближения"""
        return await self.filter({}, skip, limit, order_by="distance_au")

    async def get_fastest_approaches(self, limit: Optional[int] = None, skip: int = 0) -> List[CloseApproachModel]:
        """Получает сближения с наибольшей скоростью"""
        return await self.filter({}, skip, limit, order_by="velocity_km_s", order_desc=True)

    async def bulk_create_approaches(self, approaches_data: List[Dict[str, Any]], calculation_batch_id: str) -> int:
        """Массовое создание сближений"""
        for data in approaches_data:
            data['calculation_batch_id'] = calculation_batch_id

        created, updated = await self.bulk_create(approaches_data, "update", ["asteroid_id", "approach_time"])
    ...
    async def delete_old_approaches(self, cutoff_date: datetime) -> int:
        ...
        return await self.bulk_delete({"approach_time__lt": cutoff_date})
