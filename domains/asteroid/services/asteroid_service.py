...
class AsteroidService(BaseService):
    ...
    async def get_by_moid(self, max_moid: float = 0.05, skip: int = 0, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Получение астероидов с MOID меньше указанного"""
        async with UnitOfWork(self.session_factory) as uow:
            asteroids = await uow.asteroid_repo.get_asteroids_by_earth_moid(max_moid, skip, limit)
    ...
    async def get_all(self, skip: int = 0, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        return await super().get_all(skip, limit or 100)
    ...
    async def get_by_orbit_class(self, orbit_class: str, skip: int = 0, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Получение астероидов по классу орбиты"""
        return await self.filter({"orbit_class": orbit_class}, skip, limit or 100, order_by="designation")

    async def get_with_accurate_diameter(self, skip: int = 0, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Получение астероидов с точными данными о диаметре"""
        return await self.filter({"accurate_diameter": True}, skip, limit or 100, order_by="estimated_diameter_km")
