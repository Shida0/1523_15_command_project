...
async def get_near_earth_asteroids(
    ...
    max_moid: float = Query(0.05, ge=0.0, le=0.5),
...
async def get_asteroids_count(asteroid_service: AsteroidService = Depends(get_asteroid_service)):
    """
    Получить общее количество астероидов.

    Возвращает общее количество астероидов в базе данных.
    """
...
async def get_asteroids_by_orbit_class(
    ...
    """
    return await asteroid_service.get_by_orbit_class(orbit_class, skip=skip, limit=limit)
...
async def get_asteroid_statistics(asteroid_service: AsteroidService = Depends(get_asteroid_service)):
    """
